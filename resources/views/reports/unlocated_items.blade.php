...
@section('title', 'Unlocated Items Report - PNOC')
@section('customcss')
   ...
   <div class="content-wrapper">
      ...
      <div class="row">
         <div class="col-lg-12 stretch-card">
            <div class="card">
               <div class="card-header">Unlocated Items Report</div>
...
<script type="text/javascript">
   $(document).ready(function() {
    ...
    $('#search-btn_submit').on('click', function (){
        var date_range = $("#date_range_pick");

        if(date_range.val() != ''){
            date_range.removeClass('is-invalid');
            date_range.next('.invalid-feedback').remove();
            $('#id-data_table').DataTable().draw(true);
        }else{
            date_range.addClass('is-invalid');
            date_range.after(`
                <span class="invalid-feedback" role="alert">
                    <strong>Please Choose the Date Range of Barcode Scanned</strong>
                </span>
            `);
        }
    ...
    function load_datatable(){
        $('#id-data_table').DataTable({
            ...
            ajax: {
                url : "{{ route('report.get_unlocated_items') }}",
                type: 'GET',
                data: function (f) {
   ...
   $(".btn-print-report").on('click', function(){
        var url         = '{{route("report.print_unlocated_items_report")}}';
        var filters     = {
    ...
    $('.btn-excel-download').on('click',function(){
	    ...
	    var url = "{{URL::to('report/export_unlocated_items_report')}}?" + $.param(query)
    ...
    $('.btn-pdf-download').on('click',function(){
	    ...
	    var url = "{{URL::to('report/generate_pdf_unlocated_items')}}?" + $.param(query)
