...
@section('title', 'Office Assets Report - PNOC')
@section('customcss')
   ...
   <div class="content-wrapper">
      ...
      <div class="row">
         <div class="col-lg-12 stretch-card">
            <div class="card">
               <div class="card-header">Asset By Offices Report</div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        <select style="border: 1px solid #aeaeae;" class="form-control" name="department" id="department">
                                            <option value="">-Department-</option>
                                        @foreach($departments as $department)
                                            <option value="{{$department->id}}">{{ucfirst($department->name)}}</option>                                            @endforeach
                                            </select>
                                    </div>
                                </div>
                            </div>
...
<script type="text/javascript">
   $(document).ready(function() {
    ...
    function load_datatable(){
        $('#id-data_table').DataTable({
            ...
            ajax: {
                url : "{{ route('report.get_office_assets') }}",
                type: 'GET',
                data: function (f) {
                    f.location_id       = $("#location").val();
                    f.department_id     = $("#department").val();
   ...
   $(".btn-print-report").on('click', function(){
        var url         = '{{route("report.print_office_report")}}';
        var filters     = {
   ...
   $('.btn-excel-download').on('click',function(){
	    ...
	    var url = "{{URL::to('report/export_office_assets')}}?" + $.param(query)
    ...
    $('.btn-pdf-download').on('click',function(){
	    ...
	    var url = "{{URL::to('report/generate_pdf_office_assets')}}?" + $.param(query)
   ...
   function load_select2(){
       ...
       $('#department').select2(select2_custom_option_static());
